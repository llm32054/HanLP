...
public abstract class Segment {
	...
	protected static List<AtomNode> atomSegment(char[] charArray,
		...
		for (int i = 0; i < charTypeArray.length; ++i) {
			...
			if (c == '.' && i + start < (charArray.length - 1)
					&& CharType.get(charArray[i + start
							+ 1]) == CharType.CT_NUM)
				charTypeArray[i] = CharType.CT_NUM;
			else if (c == '.' && i + start < (charArray.length - 1)
					&& charArray[i + start + 1] >= '0'
					&& charArray[i + start + 1] <= '9')
		...
		while (pCur < end) {
			...
			else if (pCur < end - 1 && ((nCurType == CharType.CT_SINGLE)
					|| nCurType == CharType.CT_NUM)) {
			...
			// 对于所有其它情况
				...
				atomSegment
						.add(new AtomNode(charArray[pCur], nCurType));
				pCur++;
			}
		}
	...
	protected static List<AtomNode> simpleAtomSegment(char[] charArray,
		...
		atomNodeList.add(
				new AtomNode(new String(charArray, start, end - start),
						CharType.CT_LETTER));
		return atomNodeList;
	}
	...
	protected static List<AtomNode> quickAtomSegment(char[] charArray,
		...
		while (++offsetAtom < end) {
			...
			if (curType != preType) {
				...
				if (preType == CharType.CT_NUM && "，,．."
						.indexOf(charArray[offsetAtom]) != -1) {
					if (offsetAtom + 1 < end) {
						int nextType =
								CharType.get(charArray[offsetAtom + 1]);
						if (nextType == CharType.CT_NUM) {
							continue;
						}
					}
				}
				atomNodeList.add(new AtomNode(new String(charArray,
						start, offsetAtom - start), preType));
				start = offsetAtom;
			}
			preType = curType;
		}
		if (offsetAtom == end)
			atomNodeList.add(new AtomNode(
					new String(charArray, start, offsetAtom - start),
					preType));
	 ...
	 * @return 合并后的结果
	 */
	protected static List<Vertex> combineByCustomDictionary(
		...
		if (CustomDictionary.trie != null) {
			for (int i = 1; i < length; ++i) {
				if (wordNet[i] == null)
					continue;
				BaseNode<CoreDictionary.Attribute> state =
						CustomDictionary.trie.transition(
								wordNet[i].realWord.toCharArray(), 0);
				if (state != null) {
					int to = i + 1;
					int end = to;
					...
					for (; to < length; ++to) {
						...
						state = state.transition(
								wordNet[to].realWord.toCharArray(), 0);
						if (state == null)
							break;
	 ...
	 */
	protected static List<Vertex> combineByCustomDictionary(
			List<Vertex> vertexList, final WordNet wordNetAll) {
		return combineByCustomDictionary(vertexList,
				CustomDictionary.dat, wordNetAll);
	}
						...
						new AhoCorasickDoubleArrayTrie.IHit<CoreDictionary.Attribute>() {
							...
							public void hit(int begin, int end,
								...
								if (end - begin == parentLength)
									return;
								wordNetAll.add(currentLine + begin,
										new Vertex(vertex.realWord
												.substring(begin, end),
												value));
							}
						});
	 ...
	 * @param value 新的属性
	 */
	private static void combineWords(Vertex[] wordNet, int start,
			int end, CoreDictionary.Attribute value) {
	...
	protected static List<Term> convert(List<Vertex> vertexList,
		...
		assert vertexList.size() >= 2 : "这条路径不应当短于2"
				+ vertexList.toString();
	...
	protected void mergeNumberQuantifier(List<Vertex> termList,
			WordNet wordNetAll, Config config) {
		if (termList.size() < 4)
			return;
		...
		while (iterator.hasNext()) {
			...
			if (pre.hasNature(Nature.m)) {
				...
				Vertex cur = null;
				while (iterator.hasNext() && (cur = iterator.next())
						.hasNature(Nature.m)) {
					sbQuantifier.append(cur.realWord);
					iterator.remove();
					removeFromWordNet(cur, wordNetAll, line,
							sbQuantifier.length());
				}
				if (cur != null) {
					if ((cur.hasNature(Nature.q)
							|| cur.hasNature(Nature.qv)
							|| cur.hasNature(Nature.qt))) {
						if (config.indexMode > 0) {
							wordNetAll.add(line, new Vertex(
									sbQuantifier.toString(),
				...
				if (sbQuantifier.length() != pre.realWord.length()) {
					for (Vertex vertex : wordNetAll
							.get(line + pre.realWord.length())) {
						vertex.from = null;
					}
					pre.realWord = sbQuantifier.toString();
					pre.word = Predefine.TAG_NUMBER;
					pre.attribute =
							new CoreDictionary.Attribute(Nature.mq);
	 ...
	 */
	private static void removeFromWordNet(Vertex cur,
			WordNet wordNetAll, int line, int length) {
		...
		ListIterator<Vertex> iterator =
				vertexes[line + length - cur.realWord.length()]
						.listIterator();
		while (iterator.hasNext()) {
			Vertex vertex = iterator.next();
			if (vertex == cur)
	...
	public List<Term> seg(String text) {
		...
		if (config.threadNumber > 1 && charArray.length > 10000) // 小文本多线程没意义，反而变慢了
		{
			List<String> sentenceList =
					SentencesUtil.toSentenceList(charArray);
			...
			WorkThread[] threadArray =
					new WorkThread[config.threadNumber];
			for (int i = 0; i < config.threadNumber - 1; ++i) {
				int from = i * per;
				threadArray[i] = new WorkThread(sentenceArray,
						termListArray, from, from + per);
				threadArray[i].start();
			}
			threadArray[config.threadNumber - 1] =
					new WorkThread(sentenceArray, termListArray,
							(config.threadNumber - 1) * per,
			...
			try {
				...
				logger.severe(
						"线程同步异常：" + TextUtility.exceptionToString(e));
	...
	class WorkThread extends Thread {
		...
		public WorkThread(String[] sentenceArray,
				List<Term>[] termListArray, int from, int to) {
		...
		public void run() {
			for (int i = from; i < to; ++i) {
				termListArray[i] =
						segSentence(sentenceArray[i].toCharArray());
			}
		}
	}
