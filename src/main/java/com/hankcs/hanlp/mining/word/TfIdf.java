...
public class TfIdf {
	 ...
	 * @return 一个包含词频的Map
	 */
	public static <TERM> Map<TERM, Double>
	 ...
	 */
	public static <TERM> Iterable<Map<TERM, Double>>
			tfs(Iterable<Collection<TERM>> documents, TfType type) {
		List<Map<TERM, Double>> tfs =
				new ArrayList<Map<TERM, Double>>();
	...
	public static <TERM> Map<TERM, Double> idfFromTfs(
			Iterable<Map<TERM, Double>> tfs, boolean smooth,
			boolean addOne) {
		return idf(new KeySetIterable<TERM, Double>(tfs), smooth,
				addOne);
	}
	 ...
	 * @param <VALUE> map 值类型
	 */
	static private class KeySetIterable<KEY, VALUE>
