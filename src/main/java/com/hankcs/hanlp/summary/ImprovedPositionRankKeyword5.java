 ...
 *
 */
public class ImprovedPositionRankKeyword5 extends KeywordExtractor {
	...
	public ImprovedPositionRankKeyword5(Segment defaultSegment) {
		super(defaultSegment);
	}

	public ImprovedPositionRankKeyword5() {
	}
	...
	public static List<String> getKeywordList(String document,
			int size) {
		ImprovedPositionRankKeyword5 improvedPositionRankKeyword5 =
				new ImprovedPositionRankKeyword5();

		return improvedPositionRankKeyword5.getKeywords(document, size);
	}
	...
	public static List<String> getKeywordList(String title,
		...
		return improvedPositionRankKeyword5.getKeywords(title, document,
				size);
	}
	...
	public Map<String, Float> getTermAndRank(List<Term> titleTermList,
		...
		List<String> wordNameList =
				new ArrayList<String>(termList.size());
		// 只保留文本【标题】的人名
		List<String> titleWordNameList =
				new ArrayList<String>(termList.size());
		// for循环遍历去除【正文】（除名词和形容词）其他词性词汇
		for (Term t : termList) {
			if (shouldInclude(t)) {
				wordList.add(t.word);
			}
		}
		// for循环遍历去除【标题】（除名词和形容词）其他词性词汇
		...
		// for循环保留【正文】的人名
		for (Term t : termList) {
			if (shouldIncludeName(t)) {
				wordNameList.add(t.word);
			}
		}
		// for循环保留【标题】的人名
		for (Term t : termList) {
			if (shouldIncludeName(t)) {
				titleWordNameList.add(t.word);
			}
		}
		if (Global.log) {
			System.out.println("文本【正文】去停用词及词性过滤的结果：");
			System.out.println(wordList);
			System.out.println("文本【正文】包含的人名：");
			System.out.println(wordNameList);
			System.out.println("文本【标题】去停用词及词性过滤的结果：");
			System.out.println(titleWordList);
			System.out.println("文本【标题】包含的人名：");
			System.out.println(titleWordNameList);
		}
		// 存放单词共现结果（不包含重复单词）
		Map<String, Set<String>> words =
				new TreeMap<String, Set<String>>();
		...
		// 构建【正文】中【人名】的重启概率
		if (wordNameList.size() != 0) { // 正文中包含姓名
			int position = 0;
			for (int i = 0; i < wordNameList.size(); i++) {
				// wordCountMap是否包含标题中的候选关键词
				if (!wordCountMap.containsKey(wordNameList.get(i))) { // 不包含的情况
					wordCountMap.put(wordNameList.get(i),
							(wordNameList.size() - position++) * 1.0f
									/ wordNameList.size());
				} else { // 包含的情况
					wordCountMap.put(wordNameList.get(i),
							wordCountMap.get(wordNameList.get(i))
									+ (wordNameList.size() - position++)
											* 1.0f
											/ wordNameList.size());
				}
			}
		...
		// 构建【标题】中【人名】的重启概率
		if (titleWordNameList.size() != 0) {
			int position = 0;
			for (int i = 0; i < titleWordNameList.size(); i++) {
				// wordCountMap是否包含标题中的候选关键词
				if (!wordCountMap
						.containsKey(titleWordNameList.get(i))) { // 不包含的情况
					wordCountMap.put(titleWordNameList.get(i),
							(titleWordNameList.size() - position++)
									* 1.0f / titleWordNameList.size());
				} else { // 包含的情况
					wordCountMap.put(titleWordNameList.get(i),
							wordCountMap.get(titleWordNameList.get(i))
									+ (titleWordNameList.size()
											- position++) * 1.0f
											/ titleWordNameList.size());
				}
			}
		...
		for (int i = 0; i < titleWordList.size(); i++) {
			...
			if (!wordCountMap.containsKey(titleWordList.get(i))) { // 不包含的情况
				wordCountMap.put(titleWordList.get(i), 1.0f);
			} else { // 包含的情况
				wordCountMap.put(titleWordList.get(i),
						wordCountMap.get(titleWordList.get(i)) + 1.0f);
			}
		}
		// 存放单词位置
		int position = 0;
