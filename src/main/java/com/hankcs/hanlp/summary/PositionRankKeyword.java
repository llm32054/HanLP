...
/**
 * 基于PositionRank算法的关键字提取，适用于单文档
 */
public class PositionRankKeyword extends KeywordExtractor {
	...
	public PositionRankKeyword(Segment defaultSegment) {
		super(defaultSegment);
	}

	public PositionRankKeyword() {
	}
	...
	public static List<String> getKeywordList(String document,
			int size) {
		PositionRankKeyword positionRankKeyword =
				new PositionRankKeyword();

		return positionRankKeyword.getKeywords(document, size);
	}
	...
	public Map<String, Float> getTermAndRank(List<Term> termList) {
		...
		if (Global.log) {
			System.out.println("文本【所有内容】去停用词及词性过滤的结果：");
		...
		for (String w : wordList) {
			...
			if (!wordCountMap.containsKey(w)) {
				...
				wordCountMap.put(w,
						wordCountMap.get(w) + 1.0f / position++);
			}
			// 构建words，用来存放单词的共现单词
